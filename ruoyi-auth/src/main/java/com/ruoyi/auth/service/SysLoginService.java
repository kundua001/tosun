...
public class SysLoginService
{
    ...
    public LoginUser loginApp(String mobile, String code){
        // 手机号或验证码为空 错误
        if (StringUtils.isAnyBlank(mobile, code))
        {
            remoteLogService.saveLogininfor(mobile, Constants.LOGIN_FAIL, "手机号/验证码不能为空");
            throw new BaseException("手机号/验证码不能为空");
        }
        R<LoginUser> sysUserR = remoteUserService.infoByMobile(mobile);
        if (StringUtils.isNull(sysUserR) || StringUtils.isNull(sysUserR.getData()))
        {
            remoteLogService.saveLogininfor(mobile, Constants.LOGIN_FAIL, "登录用户不存在");
            throw new BaseException("登录用户：" + mobile + " 不存在");
        }
